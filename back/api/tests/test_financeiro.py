...
class FinanceiroApiTest(APITestCase):
    def setUp(self):
        ...
        self.outro_paciente = criar_usuario('paciente_fin_b', '93000000003', 'paciente.finb@example.com', clinica=self.clinica_b)
        self.dentista_usuario = criar_usuario('dentista_fin', '93000000004', 'dentista.fin@example.com', clinica=self.clinica_a, tipo='DENTISTA')
        self.dentista_b_usuario = criar_usuario('dentista_fin_b', '93000000005', 'dentista.finb@example.com', clinica=self.clinica_b, tipo='DENTISTA')
        Dentista.objects.create(clinica=self.clinica_a, usuario=self.dentista_usuario, especialidade='Clinico', cro='93001-AL')
        Dentista.objects.create(clinica=self.clinica_b, usuario=self.dentista_b_usuario, especialidade='Clinico', cro='93002-AL')

    def criar_orcamento_com_item(self, desconto_tipo='NENHUM', desconto_valor='0.00'):
        self.client.force_authenticate(self.staff)
        orcamento = self.client.post('/api/v1/orcamentos/', {'paciente': self.paciente.id, 'titulo': 'Tratamento', 'desconto_tipo': desconto_tipo, 'desconto_valor': desconto_valor, 'total': '999.00'}, format='json')
        self.assertEqual(orcamento.status_code, status.HTTP_201_CREATED)
        item = self.client.post('/api/v1/itens-orcamento/', {'orcamento': orcamento.data['id'], 'descricao': 'Procedimento', 'quantidade': 2, 'valor_unitario': '50.00', 'subtotal': '0.01'}, format='json')
    ...
    def aprovar(self, orcamento_id):
        ...
        self.assertEqual(self.client.post(f'/api/v1/orcamentos/{orcamento_id}/aprovar/').status_code, status.HTTP_200_OK)
    ...
    def test_fluxo_calculos_parcelas_e_pagamentos(self):
        ...
        parcelas = self.client.post(f'/api/v1/orcamentos/{orcamento_id}/parcelar/', {'quantidade_parcelas': 3, 'primeiro_vencimento': str(date.today())}, format='json')
        ...
        parcial = self.client.post('/api/v1/pagamentos/', {'orcamento': orcamento_id, 'parcela': parcelas.data[0]['id'], 'valor': '10.00', 'forma_pagamento': 'PIX'}, format='json')
        integral = self.client.post('/api/v1/pagamentos/', {'orcamento': orcamento_id, 'parcela': parcelas.data[0]['id'], 'valor': '20.00', 'forma_pagamento': 'DINHEIRO'}, format='json')
        final = self.client.post('/api/v1/pagamentos/', {'orcamento': orcamento_id, 'valor': '60.00', 'forma_pagamento': 'TRANSFERENCIA'}, format='json')
    ...
    def test_desconto_valor_invalido_e_transicoes_protegidas(self):
        ...
        desconto = self.client.patch(f'/api/v1/orcamentos/{orcamento_id}/', {'desconto_tipo': 'VALOR', 'desconto_valor': '101.00'}, format='json')
    ...
    def test_permissoes_e_isolamento(self):
        ...
        criacao = self.client.post('/api/v1/orcamentos/', {'paciente': self.paciente.id, 'titulo': 'Tentativa'}, format='json')
        item = self.client.post('/api/v1/itens-orcamento/', {'orcamento': orcamento_id, 'descricao': 'Tentativa', 'quantidade': 1, 'valor_unitario': '1.00'}, format='json')
        pagamento = self.client.post('/api/v1/pagamentos/', {'orcamento': orcamento_id, 'valor': '1.00', 'forma_pagamento': 'PIX'}, format='json')
        ...
        outra_clinica = self.client.post('/api/v1/orcamentos/', {'paciente': self.paciente.id, 'titulo': 'Invasao'}, format='json')
    ...
    def test_pagamento_nao_excede_saldo_e_referencia_e_unica(self):
        orcamento_id = self.criar_orcamento_com_item()
        excesso = self.client.post('/api/v1/pagamentos/', {'orcamento': orcamento_id, 'valor': '101.00', 'forma_pagamento': 'PIX'}, format='json')
        primeiro = self.client.post('/api/v1/pagamentos/', {'orcamento': orcamento_id, 'valor': '10.00', 'forma_pagamento': 'PIX', 'referencia_externa': 'pix-123'}, format='json')
        repetido = self.client.post('/api/v1/pagamentos/', {'orcamento': orcamento_id, 'valor': '10.00', 'forma_pagamento': 'PIX', 'referencia_externa': 'pix-123'}, format='json')
