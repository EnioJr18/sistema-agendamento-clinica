...
class ProntuarioApiTest(APITestCase):
    def setUp(self):
        ...
        self.staff = criar_usuario('staff_prontuario', '81000000001', 'staff.prontuario@example.com', is_staff=True, tipo='ADMIN')
        self.paciente_a = criar_usuario('paciente_prontuario_a', '81000000002', 'paciente.a@example.com', clinica=self.clinica_a)
        self.paciente_b = criar_usuario('paciente_prontuario_b', '81000000003', 'paciente.b@example.com', clinica=self.clinica_b)
        self.usuario_dentista_a = criar_usuario('dentista_prontuario_a', '81000000004', 'dentista.a@example.com', clinica=self.clinica_a, tipo='DENTISTA')
        self.usuario_dentista_b = criar_usuario('dentista_prontuario_b', '81000000005', 'dentista.b@example.com', clinica=self.clinica_b, tipo='DENTISTA')
        self.dentista_a = Dentista.objects.create(clinica=self.clinica_a, usuario=self.usuario_dentista_a, especialidade='Clinico', cro='81001-AL')
        self.dentista_b = Dentista.objects.create(clinica=self.clinica_b, usuario=self.usuario_dentista_b, especialidade='Clinico', cro='81002-AL')
        self.prontuario_a = ProntuarioPaciente.objects.create(clinica=self.clinica_a, paciente=self.paciente_a, criado_por=self.staff)
        self.prontuario_b = ProntuarioPaciente.objects.create(clinica=self.clinica_b, paciente=self.paciente_b, criado_por=self.staff)

    def agendamento(self, *, status_agendamento=Agendamento.STATUS_EM_ATENDIMENTO, paciente=None, dentista=None, clinica=None):
        clinica = clinica or self.clinica_a
        dentista = dentista or self.dentista_a
        paciente = paciente or self.paciente_a
        inicio = timezone.now() + timedelta(days=1)
        return Agendamento.objects.create(
            clinica=clinica, dentista=dentista, paciente=paciente, procedimento='Consulta',
            data_horario=inicio, data_hora_fim=inicio + timedelta(minutes=30), duracao_minutos=30,
    ...
    def test_staff_cria_prontuario_e_rejeita_duplicado_ou_paciente_outra_clinica(self):
        ...
        criado = self.client.post('/api/v1/prontuarios/', {'clinica': self.clinica_a.id, 'paciente': novo.id}, format='json')
        duplicado = self.client.post('/api/v1/prontuarios/', {'clinica': self.clinica_a.id, 'paciente': novo.id}, format='json')
        outra_clinica = self.client.post('/api/v1/prontuarios/', {'clinica': self.clinica_a.id, 'paciente': self.paciente_b.id}, format='json')
    ...
    def test_paciente_nao_cria_nem_acessa_prontuario_de_outro_paciente(self):
        ...
        criacao = self.client.post('/api/v1/prontuarios/', {'clinica': self.clinica_a.id, 'paciente': self.paciente_a.id}, format='json')
    ...
    def test_dentista_outra_clinica_recebe_404(self):
        ...
        self.assertEqual(self.client.get(reverse('prontuario-detail', args=[self.prontuario_a.id])).status_code, status.HTTP_404_NOT_FOUND)
    ...
    def test_evolucao_permite_em_atendimento_e_concluida_e_protege_vinculos(self):
        ...
        primeira = self.client.post(url, {'prontuario': self.prontuario_a.id, 'agendamento': em_atendimento.id, 'descricao': 'Paciente em atendimento.'}, format='json')
        segunda = self.client.post(url, {'prontuario': self.prontuario_a.id, 'agendamento': concluida.id, 'descricao': 'Atendimento concluido.'}, format='json')
        ...
        imutavel = self.client.patch(reverse('evolucao-clinica-detail', args=[evolucao.id]), {'dentista': self.dentista_b.id}, format='json')
    ...
    def test_evolucao_rejeita_status_inadequado_e_atendimento_de_outro_dentista(self):
        ...
        url = '/api/v1/evolucoes-clinicas/'
        invalida = self.client.post(url, {'prontuario': self.prontuario_a.id, 'agendamento': agendada.id, 'descricao': 'Nao permitido'}, format='json')
        outra = self.client.post(url, {'prontuario': self.prontuario_a.id, 'agendamento': outro_dentista.id, 'descricao': 'Nao permitido'}, format='json')
    ...
    def test_paciente_nao_cria_nem_apaga_evolucao(self):
        evolucao = EvolucaoClinica.objects.create(
            prontuario=self.prontuario_a, agendamento=self.agendamento(), dentista=self.dentista_a,
            descricao='Registro clinico', criado_por=self.usuario_dentista_a,
        )
        self.client.force_authenticate(self.paciente_a)
        criacao = self.client.post('/api/v1/evolucoes-clinicas/', {'prontuario': self.prontuario_a.id, 'agendamento': evolucao.agendamento_id, 'descricao': 'Tentativa'}, format='json')
